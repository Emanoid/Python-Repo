...
def attache():
    ...
    files = [('All Files', '.'),
             ...
             ('Picture Files', '.png'),
             ('Photo Files', '.jpeg')] 
    filee = askopenfilename(filetypes = files, defaultextension = files)
    URL = filee
    url.set(URL)
    msg = Entry(frame,textvariable= url)
...
def text(user):
    ...
    buttonAdd = Button(frame, text="Link", command = attache)
    buttonAdd.place(x=74,y=515)   
...
Label(window, image = Logo, compound = TOP, bg = 'black').place(x=250,y=5)
